<script>
    ...
    async function importData() {
        ...
        if (await $confirmPromise("Do you want to import your backup?")) {
    ...
    async function exportData() {
        ...
        if (await $confirmPromise("Do you want to backup your data?")) {
                    ...
                    <label 
                        ...
                        on:keyup="{(e) => {
                            if(e.key === 'Enter') {
                                ...
                                const message = `${$autoUpdate ? 'Enabled' : 'Disabled'} Automated Update`
                                if ($android && false) {
                                    showToast(message)
                                } else {
                                    $toast = message
                            ...
                            <label 
                                ...
                                on:keyup="{(e) => {
                                    if(e.key === 'Enter') {
                                        ...
                                        const message = `${$autoExport ? 'Enabled' : 'Disabled'} Automated Backup`
                                        if ($android && false) {
                                            showToast(message)
                                        } else {
                                            $toast = message
...
<style>
    ...
    .option.export-switchable {
        grid-template-columns: 20px auto 20px 45px;
    }
