<script>
  ...
  async function importUserFile() {
    ...
    if (importedFile) {
      ...
      if (
        ...
        if (importFileInput instanceof Element) importFileInput.value = null;
      }
    } else {
      if (importFileInput instanceof Element) importFileInput.value = null;
    }
  ...
  async function showAllHiddenEntries() {
    ...
    if (jsonIsEmpty($hiddenMediaEntries)) {
      ...
      await $confirmPromise("Do you want to show all your hidden entries?")
    ) {
      $loadingCategory[""] = new Date().getTime();
      mediaManager({
        showId: "all",
  ...
  let isAndroidWeb, deferredPrompt, downloadAndroidApp;
  if (!$android && $mobile) {
    ...
    downloadAndroidApp = async () => {
      if (
        ...
        if (isAndroidWeb) {
          downloadLink(
            "https://github.com/u-Kuro/Kanshi-Anime-Recommender/releases/download/v9.5.4/Kanshi-v9.5.4.apk",
            "Kanshi-v9.5.4.apk",
          );
          return;
  ...
  onMount(async () => {
    ...
    if ($android) {
      $exportPathIsAvailable =
        $exportPathIsAvailable ?? (await getIDBData("exportPathIsAvailable"));
    ...
    >
      ...
      <div class="menu-options" role="group" aria-label="list actions">
        ...
        >
          ...
          <svg viewBox="0 0 512 512">
            <path
              d="M464 224h8c13 0 24-11 24-24V72a24 24 0 0 0-41-17l-42 42a224 224 0 1 0 1 317 32 32 0 0 0-45-45 160 160 0 1 1-1-227l-41 41a24 24 0 0 0 17 41h120z"
            ></path>
          </svg>
      ...
      <div class="menu-options" role="group" aria-label="backup and restore">
        <div
          ...
          role="menuitem"
        >
          <svg viewBox="0 0 448 512" style:--width="{'18px'}">
            <path
              d="M247 9a32 32 0 0 0-46 0L73 137a32 32 0 0 0 46 46l73-74v211a32 32 0 1 0 64 0V109l73 74a32 32 0 0 0 46-46L247 9zM64 352a32 32 0 1 0-64 0v64c0 53 43 96 96 96h256c53 0 96-43 96-96v-64a32 32 0 1 0-64 0v64c0 18-14 32-32 32H96c-18 0-32-14-32-32v-64z"
            ></path>
          </svg>
        ...
        >
          <svg viewBox="0 0 512 512">
            <path
              d="M288 32a32 32 0 1 0-64 0v243l-73-74a32 32 0 0 0-46 46l128 128c13 12 33 12 46 0l128-128a32 32 0 0 0-46-46l-73 74V32zM64 352c-35 0-64 29-64 64v32c0 35 29 64 64 64h384c35 0 64-29 64-64v-32c0-35-29-64-64-64H347l-46 45a64 64 0 0 1-90 0l-45-45H64zm368 56a24 24 0 1 1 0 48 24 24 0 1 1 0-48z"
            ></path>
          </svg>
          ...
          {#if $android}
            <div class="change-directory">
              <svg
                ...
                on:keyup="{(e) =>
                  e.key === 'Enter' && handleExportDirectory(e)}"
              >
                <path
                  d="M89 224 0 376V96c0-35 29-64 64-64h118c17 0 33 7 45 19l26 26c12 12 29 19 46 19h117c35 0 64 29 64 64v32H144c-23 0-44 12-55 32zm27 16c6-10 17-16 28-16h400c12 0 22 6 28 16s5 22 0 32L460 464c-6 10-17 16-28 16H32c-11 0-22-6-28-16s-5-22 0-32l112-192z"
                ></path>
              </svg>
            </div>
      ...
      <div class="menu-options" role="group" aria-label="tool configs">
        {#if $android}
          <div
            ...
            on:click="{(e) => {
              const target = e.target;
              const classList = target.classList;
              if (classList.contains('switch') || target.closest('.switch'))
                return;
              $keepAppRunningInBackground = !$keepAppRunningInBackground;
            }}"
            on:keyup="{() => {}}"
          >
            <svg viewBox="0 0 640 512" style:--width="{'25px'}">
              <path
        ...
        <div
          ...
          on:keyup="{() => {}}"
        >
          <svg viewBox="0 0 384 512" style:--width="{'18px'}">
            <path
              d="M272 384c10-32 30-59 49-86l16-22a176 176 0 1 0-289 0c4 8 10 15 15 22 20 27 40 54 49 86h160zm-80 128c44 0 80-36 80-80v-16H112v16c0 44 36 80 80 80zm-80-336c0 9-7 16-16 16s-16-7-16-16c0-62 50-112 112-112 9 0 16 7 16 16s-7 16-16 16c-44 0-80 36-80 80z"
            ></path>
          </svg>
      ...
      {#if $android}
        ...
        <div class="menu-options" role="group" aria-label="tool actions">
          ...
          >
            <svg viewBox="0 0 576 512">
              <path
                d="M421 302a24 24 0 1 1 24-24 24 24 0 0 1-24 24m-265 0a24 24 0 1 1 24-24 24 24 0 0 1-24 24m273-145 48-83a10 10 0 1 0-17-10l-49 84a301 301 0 0 0-246 0l-49-83a10 10 0 1 0-17 10l48 83A283 283 0 0 0 0 384h576c-8-98-64-182-147-227"
              ></path>
            </svg>
          ...
          >
            <svg viewBox="0 0 448 512" style:--width="{'17px'}">
              <path
                d="m135 18-7 14H32a32 32 0 1 0 0 64h384a32 32 0 1 0 0-64h-96l-7-14c-6-11-17-18-29-18H164c-12 0-23 7-29 18zm281 110H32l21 339c2 25 23 45 48 45h246c25 0 46-20 48-45l21-339z"
              ></path>
            </svg>
      ...
      <div class="menu-options" role="group" aria-label="other option">
        {#if $mobile && !$android && (isAndroidWeb || (deferredPrompt && typeof deferredPrompt?.prompt === "function"))}
          ...
          >
            ...
            {:else}
              <svg viewBox="0 0 384 512" style:--width="{'18px'}">
                <path
                  d="M16 64C16 29 45 0 80 0h224c35 0 64 29 64 64v384c0 35-29 64-64 64H80c-35 0-64-29-64-64V64zm208 384a32 32 0 1 0-64 0 32 32 0 1 0 64 0zm80-384H80v320h224V64z"
                ></path>
              </svg>
        ...
        >
          <svg viewBox="0 0 640 512">
            <path
              d="M96 128a128 128 0 1 1 256 0 128 128 0 1 1-256 0zM0 482c0-98 80-178 178-178h92c98 0 178 80 178 178 0 17-13 30-30 30H30c-17 0-30-13-30-30zm504-170v-64h-64a24 24 0 1 1 0-48h64v-64a24 24 0 1 1 48 0v64h64a24 24 0 1 1 0 48h-64v64a24 24 0 1 1-48 0z"
            ></path>
          </svg>
        ...
        >
          <svg viewBox="0 0 512 512">
            <path
              d="M256 32c14 0 27 8 35 20l216 368a40 40 0 0 1-35 60H40a40 40 0 0 1-34-60L222 52c7-12 20-20 34-20zm0 128c-13 0-24 11-24 24v112a24 24 0 1 0 48 0V184c0-13-11-24-24-24zm32 224a32 32 0 1 0-64 0 32 32 0 1 0 64 0z"
            ></path>
          </svg>
