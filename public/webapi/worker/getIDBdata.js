...
self.onmessage = async ({ data }) => {
    try {
        if (!db) await IDBinit()
        if (data.name === "mediaEntriesIsEmpty") {
            self.postMessage(jsonIsEmpty(await retrieveJSON("mediaEntries")))
            ...
            self.postMessage(jsonIsEmpty(await retrieveJSON("recommendedMediaList")))
