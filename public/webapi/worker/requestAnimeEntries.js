...
const excludedFormats = { music: true, manga: true, novel: true, one_shot: true }
...
self.onmessage = async ({ data }) => {
    ...
    if (isJsonObject(animeEntries) && jsonIsEmpty(animeEntries)) {
        self.postMessage({ getEntries: true })
    } else {
        getNewEntries(animeEntries, excludedEntries) // Start Call
    }
    animeEntries = null
    ...
    async function getNewEntries(animeEntries, excludedEntries) {
        self.postMessage({ progress: 0 })
        let currentAnimeIDs = Object.keys(animeEntries).concat(Object.keys(excludedEntries)).join(',')
        let entriesCount = 0
            ...
            fetch('https://graphql.anilist.co', {
                ...
                .then(async ({ result, headers }) => {
                    ...
                    if (typeof (error = result?.errors?.[0]?.message) === "string") {
                        ...
                        let hasExcludedEntry = false
                        let newAddedEntries = 0
                            ...
                            media.forEach((anime) => {
                                if (typeof anime?.id === "number") {
                                    if ((
                                        ...
                                        || !excludedFormats[anime?.format?.trim?.()?.toLowerCase?.()]
                                    ) && !anime?.genres?.some?.((genre) => genre?.trim?.()?.toLowerCase?.() === "hentai")
                                    ) {
                                        ...
                                        if (excludedEntries?.hasOwnProperty?.(anime.id)) {
                                            delete excludedEntries[anime.id]
                                            hasExcludedEntry = true
                                        } else if (!animeEntries?.hasOwnProperty?.(anime.id)) {
                                            anime.dateAdded = new Date().getTime()
                                        }
                                        animeEntries[anime.id] = anime
                                        hasUpdatedEntry = true
                                        newAddedEntries++
                                    } else {
                                        if (animeEntries?.hasOwnProperty?.(anime.id)) {
                                            newAddedEntries--
                                            delete animeEntries[anime.id]
                                            hasUpdatedEntry = true
                                        }
                                        excludedEntries[anime.id] = 1
                                        hasExcludedEntry = true
                                    }
                                }
                            })
                        ...
                        if (hasNextPage && media.length > 0) {
                            ...
                            entriesCount = entriesCount + Math.max(0, newAddedEntries)
                            if (entriesCount > 0) {
                                self.postMessage({ status: `${entriesCount} anime has been added` })
                            }
                            ...
                            if (onlyGetNewEntries) {
                                ...
                                updateAiringAnime(animeEntries, excludedEntries)
    ...
    async function updateAiringAnime(animeEntries, excludedEntries) {
        self.postMessage({ progress: 0 })
                    ...
                    if (typeof (error = result?.errors?.[0]?.message) === "string") {
                        ...
                        if (hasNextPage && media.length > 0 &&
                            ...
                            if (pastAiringEpisodeIDs.length > 0) {
                                ...
                                updateNonRecentEntries(animeEntries, excludedEntries, lastAnimeUpdate)
                                excludedEntries = animeEntries = null
                            }
                        }
                    }
        ...
        function recallUNAE(page) {
            fetch('https://graphql.anilist.co', {
                ...
                body: JSON.stringify({
                    query: `{Page(page:${page},perPage:${maxAnimePerPage}){pageInfo{hasNextPage}media(type:ANIME,id_in:[${pastAiringEpisodeIDsString || ''}]){id nextAiringEpisode{episode airingAt}}}}`
                })
            })
                .then(async (response) => {
                    let headers = response.headers
    ...
    async function updateNonRecentEntries(animeEntries, excludedEntries, lastAnimeUpdate) {
        ...
        let excludedAnimeIds = Object.keys(excludedEntries).join(',')
        ...
        function recallGOUD() {
            ...
            fetch('https://graphql.anilist.co', {
                ...
                body: JSON.stringify({
                    query: `{Page(page:1,perPage:1){media(type:ANIME,id_not_in:[${excludedAnimeIds}],sort:[UPDATED_AT]){updatedAt}}}`
                })
            })
                .then(async (response) => {
                    let headers = response.headers
        ...
        function recallUNRE(page) {
            fetch('https://graphql.anilist.co', {
                ...
                body: JSON.stringify({
                    query: `{Page(page:${page},perPage:${maxAnimePerPage}){pageInfo{hasNextPage}media(type:ANIME,id_not_in:[${excludedAnimeIds}],sort:[UPDATED_AT_DESC]){id updatedAt title{romaji english userPreferred}relations{edges{relationType node{id popularity}}}description siteUrl averageScore episodes duration trending popularity favourites format genres status coverImage{large}trailer{id thumbnail site}bannerImage tags{name rank category}studios{nodes{name siteUrl isAnimationStudio}}startDate{month day year}endDate{month day year}seasonYear season staff(perPage:${maxStaffPerPage},page:1,sort:[RELEVANCE]){pageInfo{hasNextPage}edges{node{name{userPreferred}siteUrl}role}}nextAiringEpisode{episode airingAt}}}}`
                })
            })
                .then(async (response) => {
                    let headers = response.headers
