...
self.onmessage = async ({
    ...
    if (data?.hasOwnProperty("minimizeTransaction")) {
        ...
        if (isJsonObject(mediaEntries) && !jsonIsEmpty(mediaEntries)) {
            getNewEntries(
                mediaEntries,
                await retrieveJSON("excludedEntries") || {},
                await retrieveJSON("mediaUpdateAt") || 1706674120
            )
        } else {
            self.postMessage({
                error: "No Media Entries were Found"
            })
        }
    }
                ...
                if (typeof (error = result?.errors?.[0]?.message) === "string") {
                    ...
                    if (!foundLastHighestID && newLowestID > lastHighestID) {
                        ...
                        if (foundLastHighestID) {
                            ...
                            if (jsonIsEmpty(mediaEntries) || !isJsonObject(mediaEntries)) {
                                self.postMessage({
                                    error: "No Media Entries were Found"
                                })
                                return
