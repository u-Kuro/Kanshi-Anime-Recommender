...
self.onmessage = async ({
    ...
    self.postMessage({
        status: "Exporting User Data"
    });
    const importantData = await getIDBDataAsBlob(["username", "categories", "excludedMediaIds", "mediaEntriesInfo", "mediaEntries"]);
    if (!(importantData.categories instanceof Blob) || !(importantData.excludedMediaIds instanceof Blob) || !(importantData.mediaEntriesInfo instanceof Blob) || !(importantData.mediaEntries instanceof Blob)) {
        self.postMessage({
            missingData: true
        });
        return
    }
    let backUpData = Object.assign(importantData, await getIDBDataAsBlob(["mediaUpdateAt", "hiddenMediaEntries", "algorithmFilters", "mediaCautions", "userMediaEntries", "userMediaUpdateAt", "tagInfo", "tagInfoUpdateAt"]));
    for (const key in backUpData) {
        if (!(backUpData[key] instanceof Blob)) {
    ...
    if (data === "android") {
        ...
        self.postMessage({
            blob: backUpData,
            username: importantData.username instanceof Blob ? await decompressBlobToJSON(importantData.username) : null
        })
    } else {
...
function setIDBData(key, value) {
    return new Promise((resolve, reject) => {
        try {
            ...
            const put = transaction.objectStore(key).put(value, key);
            put.onerror = ex => {
                console.error(ex);
                reject(ex);
                transaction.abort()
...
async function compressBlobs(blobsWithKeys) {
    ...
    for (const [key, blob] of Object.entries(blobsWithKeys)) {
        ...
        totalSize += 4 + keyBlob.size + 4 + blob.size
    }
...
async function decompressBlobToJSON(blob) {
    return await new Response(blob.stream().pipeThrough(new DecompressionStream("gzip"))).json()
}