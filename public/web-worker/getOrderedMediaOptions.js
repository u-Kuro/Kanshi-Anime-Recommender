...
self.onmessage = async ({
    ...
    if (isJsonObject(mediaOptions) && !jsonIsEmpty(mediaOptions)) {
        ...
        const algorithmFilterSelections = {
            Genre: 1,
            Tag: 1,
            "Tag Category": 1
        }
        for (let i = 0; i < orderedFiltersKeys.length; i++) {
            const filterKey = orderedFiltersKeys[i],
                isAlgorithmFilterSelection = algorithmFilterSelections[filterKey]
            ...
            if (optionSort === "descnum") {
                ...
                options.sort((a, b) => a.localeCompare(b, undefined, {
                    sensitivity: "base"
                })) // default ascending
            }
            if (isAlgorithmFilterSelection) {
        ...
        orderedMediaOptions = await compressJSONToBlob(orderedMediaOptions)
        await setIDBData("orderedMediaOptions", orderedMediaOptions)
        self.postMessage(data ? orderedMediaOptions : { done: true })
    } else {
        self.postMessage({
