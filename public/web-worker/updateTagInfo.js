...
self.onmessage = async ({
    ...
    if (tagInfoUpdateAt > 0 && hasTagInfoData(tagInfo)) {
        ...
        if (shouldUpdateTagInfo) {
            getTagInfoData(tagInfo, data?.getData)
        } else {
            self.postMessage({
                done: true
            })
        ...
        getTagInfoData(null, data?.getData)
    }
};
async function getTagInfoData(tagInfo, getData) {
    try {
        ...
        if (isJsonObject(tagInfo) && !jsonIsEmpty(tagInfo)) {
            ...
            if (getData) {
                self.postMessage(await compressJSONToBlob(tagInfo))
            } else {
                self.postMessage({
                    done: true
                })
        ...
        if (!await isConnected()) {
            ...
            setTimeout(() => getTagInfoData(tagInfo, getData), 6e4)
        }
    }
}
