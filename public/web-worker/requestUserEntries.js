...
self.onmessage = async ({
    ...
    const {
        username,
        userMediaUpdateAt
    } = await getIDBRecords(["username", "userMediaUpdateAt"]);
    ...
    function recallUE(username) {
        if (typeof userMediaUpdateAt === "number" && !isNaN(userMediaUpdateAt) && isFinite(userMediaUpdateAt) && userMediaUpdateAt <= Number.MAX_SAFE_INTEGER) {
            if (!visibilityChange) {
                self.postMessage({
            ...
            fetch("https://graphql.anilist.co", {
                ...
                if (typeof(error = result?.errors?.[0]?.message) === "string") {
                    ...
                    if (typeof currentUserMediaUpdateAt === "number" && !isNaN(currentUserMediaUpdateAt) && isFinite(currentUserMediaUpdateAt) && currentUserMediaUpdateAt <= Number.MAX_SAFE_INTEGER) {
        ...
        function recallAV(chunk) {
            fetch("https://graphql.anilist.co", {
                ...
                if (typeof(error = result?.errors?.[0]?.message) === "string") {
                    ...
                    if (typeof newUserMediaAnimeUpdate === "number" && !isNaN(newUserMediaAnimeUpdate) && isFinite(newUserMediaAnimeUpdate) && newUserMediaAnimeUpdate <= Number.MAX_SAFE_INTEGER) {
                        if (newUserMediaAnimeUpdate > currentUserMediaUpdate || typeof currentUserMediaUpdate !== "number" || isNaN(currentUserMediaUpdate) || !isFinite(currentUserMediaUpdate) || !(currentUserMediaUpdate <= Number.MAX_SAFE_INTEGER)) {
                            currentUserMediaUpdate = newUserMediaAnimeUpdate
                        }
                    }
                    const newUserMediaMangaUpdate = mangaCollection?.user?.updatedAt;
                    if (typeof newUserMediaMangaUpdate === "number" && !isNaN(newUserMediaMangaUpdate) && isFinite(newUserMediaMangaUpdate) && newUserMediaMangaUpdate <= Number.MAX_SAFE_INTEGER) {
                        if (newUserMediaMangaUpdate > currentUserMediaUpdate || typeof currentUserMediaUpdate !== "number" || isNaN(currentUserMediaUpdate) || !isFinite(currentUserMediaUpdate) || !(currentUserMediaUpdate <= Number.MAX_SAFE_INTEGER)) {
                    ...
                    if (hasNextChunk && (userEntries?.length ?? 0) > 0) {
                        ...
                        if (typeof currentUserMediaUpdate === "number" && !isNaN(currentUserMediaUpdate) && isFinite(currentUserMediaUpdate) && currentUserMediaUpdate <= Number.MAX_SAFE_INTEGER) {
