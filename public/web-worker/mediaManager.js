...
self.onmessage = async ({
    ...
    try {
        self.postMessage({
            status: "Updating Categories and List"
        })
        ...
        const recordsToSet = await getIDBRecords([
            "categories",
            "mediaCautions",
            "hiddenMediaEntries"
        ])
        ...
        if (categoriesToEdit instanceof Array && categoriesToEdit.length > 0) {
            ...
            for (let i = 0; i < categoriesToEdit.length; i++) {
                ...
                if (action?.add) {
                    ...
                    if (objectEntriesIsMoreThanOne(recordsToSet.categories)) {
        ...
        self.postMessage(await compressJSONToBlob(messageToPost))
    } catch (reason) {
        console.error(reason)
        let error = reason?.stack || reason?.message
        if (typeof error !== "string" || !error) {
...
function objectEntriesIsMoreThanOne(records) {
    let i = 0
                        ...
                        if (e?.relationType?.toLowerCase?.() === "prequel") {
                            ...
                            if (mediaRelationID != null) {
                                ...
                                if (
                                    ...
                                    return relationMedia?.popularity < media?.popularity;
                                }
                            }
                        }
                    });
    ...
    if (sortType === "desc") {
        if (sortName === "Weighted Score") {
            ...
            mediaList.sort((a, b) => {
                ...
                let dateA = getJapaneseStartDate({
                    season,
                    year,
                    month,
                    day
                ...
                day = startDate?.day
                let dateB = getJapaneseStartDate({
                    season,
                    year,
                    month,
                    day
