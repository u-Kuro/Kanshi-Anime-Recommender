...
self.onmessage = async ({
    ...
    if (data?.hasOwnProperty("minimizeTransaction")) {
        ...
        const {
            ...
            mediaEntriesInfo,
            excludedMediaIds = {},
            mediaUpdateAt = 1730019637
        } = await getIDBRecords(["mediaEntries", "mediaEntriesInfo", "excludedMediaIds", "mediaUpdateAt"]);
        if (isJsonObject(mediaEntries) && isJsonObject(mediaEntriesInfo) && !jsonIsEmpty(mediaEntries) && !jsonIsEmpty(mediaEntriesInfo)) {
            getNewEntries(mediaEntries, mediaEntriesInfo, excludedMediaIds, mediaUpdateAt)
        } else {
            self.postMessage({
                error: "No Media Entries was Found"
            })
        }
    }
    async function getNewEntries(mediaEntries, mediaEntriesInfo, excludedMediaIds, mediaUpdateAt) {
                ...
                if (typeof(error = result?.errors?.[0]?.message) === "string") {
                    ...
                    if (medias instanceof Array && medias.length) {
                        for (let media of medias) {
                            ...
                            if (isValidNumber(currentId)) {
                                ...
                                if ((typeof media?.format !== "string" || !excludedFormats[loweredFormat]) && !media?.genres?.some?.(genre => genre?.trim?.()?.toLowerCase?.() === "hentai")) {
                                    ...
                                    mediaEntriesInfo[currentId] = {
                                        description: description,
                                        duration: duration,
                                        trailer: trailer,
                                        bannerImage: bannerImage
                                    };
                                    mediaEntries[currentId] = otherProps;
                                    shouldUpdateMediaEntries = true
                                } else {
                                    if (mediaEntries.hasOwnProperty(currentId) || mediaEntriesInfo.hasOwnProperty(currentId)) {
                                        delete mediaEntriesInfo[currentId];
                    ...
                    if (!foundLastHighestID && newLowestID > lastHighestID) {
                        ...
                        if (foundLastHighestID) {
                            if (highestIncludedEntryID <= highestExcludedEntryID) {
                                ...
                                if (shouldUpdateMediaEntries) {
                                    ...
                                    recordsToSet.mediaEntriesInfo = mediaEntriesInfo;
                            ...
                            ({
                                ...
                                mediaEntriesInfo,
                                mediaUpdateAt = 1730019637
                            } = await getIDBRecords(["mediaEntries", "mediaEntriesInfo", "mediaUpdateAt"]));
                            if (jsonIsEmpty(mediaEntries) || jsonIsEmpty(mediaEntriesInfo) || !isJsonObject(mediaEntries) || !isJsonObject(mediaEntriesInfo)) {
                                self.postMessage({
                                    error: "No Media Entries was Found"
                                });
                                return
                        ...
                        if (onlyGetNewEntries) {
                            ...
                            updateMediaEntries(mediaEntries, mediaEntriesInfo, excludedMediaIds, mediaUpdateAt)
                        }
                    }
                }
            }).catch(async error => {
    ...
    async function updateMediaEntries(mediaEntries, mediaEntriesInfo, excludedMediaIds, mediaUpdateAt) {
        ...
        let pastAiringEpisodeIDs = mediaEntriesArray.filter(({
            ...
            if (isJsonObject(nextAiringEpisode)) {
                ...
                return releaseDate instanceof Date && !isNaN(releaseDate) && releaseDate <= (new Date).getTime();
            } else {
                return false
            }
        }).map(({
        ...
        let nonUpdatedMediaIDs = mediaEntriesArray.filter(({
                                                               updatedAt
                                                           }) => updatedAt <= mediaUpdateAt || !isValidNumber(updatedAt)).sort((a, b) => {
                    ...
                    if (medias instanceof Array) {
                        for (let media of medias) {
                            ...
                            if (isValidNumber(currentId)) {
                                ...
                                if ((typeof media?.format !== "string" || !excludedFormats[media?.format?.trim?.()?.toLowerCase?.()]) && !media?.genres?.some?.(genre => genre?.trim?.()?.toLowerCase?.() === "hentai")) {
                                    if (excludedMediaIds?.hasOwnProperty?.(currentId)) {
                                        delete excludedMediaIds[currentId];
                                        shouldUpdateExcludedMediaIds = true
                                    }
                                    const savedMedia = mediaEntries[currentId];
                                    if (isJsonObject(savedMedia)) {
                                        const isPossiblyFinished = isValidNumber(savedMedia?.nextAiringEpisode?.episode) && savedMedia?.nextAiringEpisode?.episode === savedMedia?.episodes && isValidNumber(savedMedia?.nextAiringEpisode?.airingAt) && new Date(savedMedia?.nextAiringEpisode?.airingAt * 1e3) <= new Date;
                                        const newStatusIsStillReleasing = media?.status?.trim?.()?.toLowerCase?.() === "releasing" && savedMedia?.status?.trim?.()?.toLowerCase?.() === "releasing";
                                        const newNextAiringEpisodeIsRemoved = !isJsonObject(media?.nextAiringEpisode);
                                        const updateNextAiringEpisode = !(isPossiblyFinished && newStatusIsStillReleasing && newNextAiringEpisodeIsRemoved);
                        ...
                        for (let media of medias) {
                            ...
                            if (isValidNumber(currentId)) {
                                ...
                                if ((typeof media?.format !== "string" || !excludedFormats[loweredFormat]) && !media?.genres?.some?.(genre => genre?.trim?.()?.toLowerCase?.() === "hentai")) {
                                    ...
                                    const savedMedia = mediaEntries[currentId];
                                    const savedMediaInfo = mediaEntriesInfo[currentId];
                                    if (isJsonObject(savedMedia) && isJsonObject(savedMediaInfo)) {
                                        ...
                                        if (!isEditedEntry) {
                                            ...
                                            if (newTrailerId) {
                                                let oldTrailerId = savedMediaInfo.trailer?.id;
                                        ...
                                        const isPossiblyFinished = isValidNumber(savedMedia?.nextAiringEpisode?.episode) && savedMedia?.nextAiringEpisode?.episode === savedMedia?.episodes && isValidNumber(savedMedia?.nextAiringEpisode?.airingAt) && new Date(savedMedia?.nextAiringEpisode?.airingAt * 1e3) <= new Date;
                                        const newStatusIsStillReleasing = loweredStatus === "releasing" && savedMedia?.status?.trim?.()?.toLowerCase?.() === "releasing";
                                        const newNextAiringEpisodeIsRemoved = !isJsonObject(media?.nextAiringEpisode);
                                        const dontUpdateNextAiringEpisode = isPossiblyFinished && newStatusIsStillReleasing && newNextAiringEpisodeIsRemoved;
...
function IDBInit() {
    return new Promise((resolve, reject) => {
        try {
            const request = indexedDB.open("Kanshi.Media.Recommendations.AniList.W~uPtWCq=vG$TR:Zl^#t<vdS]I~N70", 1);
            request.onsuccess = ({
                                     target
                                 }) => {
                db = target.result;
        ...
        try {
            for (const key in records) {
                if (isJsonObject(records[key]) || records[key] instanceof Array) {
                    records[key] = await new Response(new Blob([JSON.stringify(records[key])]).stream().pipeThrough(new CompressionStream("gzip"))).blob()
                } else if (records[key] instanceof Blob) {
                    records[key] = await new Response(records[key].stream().pipeThrough(new CompressionStream("gzip"))).blob()
                }
            }
            const transaction = db.transaction(Object.keys(records), "readwrite");
            for (const key in records) {
                const put = transaction.objectStore(key).put(records[key], key);
                put.onerror = ex => {
                    console.error(ex);
                    reject(ex);
                    transaction.abort()
...
function getIDBData(key) {
    return new Promise(resolve => {
        try {
            const get = db.transaction(key, "readonly").objectStore(key).get(key);
            get.onsuccess = async () => {
                let value = get.result;
                if (value instanceof Blob) {
                    value = await new Response(value.stream().pipeThrough(new DecompressionStream("gzip"))).json()
                }
                resolve(value)
            };
            get.onerror = ex => {
...
function getIDBRecords(recordKeys) {
    return new Promise(async resolve => {
        try {
            ...
            resolve(Object.fromEntries(await Promise.all(recordKeys.map(key => {
                return new Promise(resolve => {
                    const get = transaction.objectStore(key).get(key);
                    get.onsuccess = async () => {
                        let value = get.result;
                        if (value instanceof Blob) {
                            value = await new Response(value.stream().pipeThrough(new DecompressionStream("gzip"))).json()
