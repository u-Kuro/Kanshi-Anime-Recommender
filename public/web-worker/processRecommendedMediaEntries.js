...
self.onmessage = async ({
    ...
    try {
        ...
        self.postMessage({
            status: "Processing Recommendation List"
        })
        const {
            mediaEntries = {},
            userMediaEntries = []
        ...
        let usedScoreBasis, maxScore = -Infinity,
            minScore = Infinity,
            maxWeightedScore = -Infinity,
            minWeightedScore = Infinity;
        let meanUserScore, meanScoreAll, meanScoreAbove;
        const mediaEntriesArray = Object.values(mediaEntries ?? {});
        let averageScoresArray = [],
            maxAverageScore = -Infinity,
            animePopularityArray = [],
            mangaPopularityArray = [],
            novelPopularityArray = []
        ...
        if (
            ...
            for (let i = 0; i < mediaEntriesArray.length; i++) {
                ...
                const media = mediaEntriesArray[i]
                let {
                    mediaID, mediaUrl, format,
                    countryOfOrigin, year, season,
                    genres, tags, studios, status,
                } = getCommonMediaData(media)
                ...
                for (let j = 0; j < tags.length; j++) {
                    ...
                    if (tag && mediaOptions.Tag[tag] === undefined) {
                        mediaOptions.Tag[tag] = true
                    }
                    if (tagCategory && mediaOptions["Tag Category"][tagCategory] === undefined) {
                        mediaOptions["Tag Category"][tagCategory] = true
                    }
                }
                let includedStudios = {};
                for (let j = 0; j < studios.length; j++) {
                    ...
                    if (studio && mediaOptions.Studio[studio] === undefined) {
                        mediaOptions.Studio[studio] = true
                    }
                }
                ...
                if (
                    ...
                    if (airingAtDate <= currentDate && media.nextAiringEpisode.episode === episodes) {
                    ...
                    const possibleAiringDate = getJapaneseStartDate({
                        season,
                        year,
                        month,
                        day
            ...
            let {
                standardDeviation,
                mean
            } = arrayMeanAndSTD(averageScoresArray)
            if (
                customUserScoreBase > 0 &&
...
function getCommonMediaData(media) {
    return {
        mediaID: media?.id,
        mediaUrl: media?.siteUrl,
        format: capitalizeWords(media?.format),
        countryOfOrigin: media?.countryOfOrigin,
        year: media?.seasonYear || media?.startDate?.year || media?.endDate?.year,
        season: capitalizeWords(media?.season),
        genres: media?.genres || [],
        tags: media?.tags || [],
        studios: media?.studios?.edges?.filter?.((e) => e?.isMain) || [],
        status: capitalizeWords(media?.status)
    }
}
...
function mapValue(originalValue, lowestValue, highestValue, newLowestRange, newHighestRange) {
    return ((originalValue - lowestValue) * (newHighestRange - newLowestRange) / (highestValue - lowestValue)) + newLowestRange;
}
