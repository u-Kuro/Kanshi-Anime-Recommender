...
async function executeMessage() {
    ...
    try {
        ...
        if (hasOwnProperty.call(data, "loadMore")) {
                ...
                loadMediaTimeout[selectedCategory] = setTimeout(() => {
                    ...
                    if (category != null) {
                        ...
                        if (idx < mediaListLen && (id = loadedMediaList[selectedCategory][idx])) {
                            const media = editMedia(id, category.shownSortName)
            ...
            if (categories != null && categorySelected != null) {
                ...
                if (category != null) {
                    ...
                    self.postMessage(await compressJSONToBlob({
                        categorySelected,
                        category: {
                            mediaFilters,
                            sortBy,
        ...
        } else if (hasOwnProperty.call(data, "updateMediaNotifications")) {
            try {
                if (data.initList === true) { recommendedMediaEntries = await getIDBData("recommendedMediaEntries") }
                if (recommendedMediaEntries != null) {
                    const mediaIds = await decompressBlobToJSON(data.mediaIdsBlob)
                    const mediaUpdates = {}
                    for (let i = 0; i < mediaIds.length; i++) {
                        const media = recommendedMediaEntries[mediaIds[i]]
                        if (media) {
                    ...
                    if (jsonIsEmpty(mediaUpdates)) {
                        ...
                        self.postMessage({
                            updateMediaNotifications: true,
                            mediaUpdatesBlob: await compressBlob(
                                new Blob([JSON.stringify(Object.values(mediaUpdates))])
                            ),
                            postId
                        });
                    }
            ...
            if (!isJsonObject(categories) || jsonIsEmpty(categories)) {
                ...
                updateMediaCautions(mediaCautions)
                ...
                self.postMessage({ shouldUpdateMediaNotifications: true})
            }
        } else {
            ...
            const updateList = (updateDate, updatedCategories) => {
                ...
                if (shouldUpdateRecommendedMediaList) {
                    promises.push((async () => {
                        ({
                            recommendedMediaEntries = recommendedMediaEntries,
                            mediaEntriesInfo = mediaEntriesInfo
                        } = await getIDBRecords([
                            "recommendedMediaEntries",
                            "mediaEntriesInfo"
                        ]));
                ...
                if (shouldUpdateMediaCautions) {
                    promises.push((async () => {
                        ...
                        if (mediaCautions instanceof Array) {
                            ...
                            updateMediaCaution(mediaCautions)
                        }
...
function updateMediaCaution(mediaCautions) {
...
function editMedia(id, shownSortName) {
    let media, mediaInfo
    if (
        (media = recommendedMediaEntries[id]) == null ||
        (mediaInfo = mediaEntriesInfo[id]) == null
    ) return null
    ...
    media = Object.assign(
        JSON.parse(JSON.stringify(media)), {
            ...
            trailerID: mediaInfo.trailer?.id,
            bannerImageUrl: mediaInfo.bannerImage,
            trailerThumbnailUrl: mediaInfo.trailer?.thumbnail,
        }
    )
    ...
    if (tags?.length) {
        ...
        tagCaution.sort(sortCaution)
        tagCaution = tagCaution.map(({
                                         tagName,
                                         score,
                                         tagRank
                                     }) => {
        ...
        tagSemiCaution.sort(sortCaution)
        tagSemiCaution = tagSemiCaution.map(({
                                                 tagName,
                                                 score,
                                                 tagRank
                                             }) => {
        ...
        tagsFavourite = tagsFavourite.map(({
                                               tagName,
                                               score,
                                               tagRank
                                           }) => {
        ...
        tagsOthers.sort(sortCaution);
        tagsOthers = tagsOthers.map(({
                                         tagName,
                                         score,
                                         tagRank
                                     }) => {
...
function formatNumber(number, dec = 2) {
    if (typeof number === "number") {
        ...
        if (Math.abs(number) >= 1000) {
            ...
            return formattedNumber || number.toLocaleString("en-US", {
...
async function compressJSONToBlob(json) {
    return await compressBlob(new Blob([JSON.stringify(json)]))
}