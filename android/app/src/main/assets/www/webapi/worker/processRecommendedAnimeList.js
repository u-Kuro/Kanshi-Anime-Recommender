...
self.onmessage = async ({ data }) => {
    ...
    let selectedCustomFilter = data?.selectedCustomFilter || await retrieveJSON("selectedCustomFilter") || "Custom Filter"
    let activeTagFilters = data?.activeTagFilters || await retrieveJSON("activeTagFilters")
    let algorithmFilter = activeTagFilters?.[selectedCustomFilter]?.['Algorithm Filter'] || []
    ...
    self.postMessage({ status: "Updating Anime Franchise" })
    ...
    for (let i = 0; i < animeEntries.length; i++) {
        ...
        let afIdx;
        if (afIdxs.length === 1) { // if there is only one
            // Index of Franchise
        ...
        } else { // else if its empty or not alone
    ...
    for (let i = 0; i < userEntries.length; i++) {
        ...
        if (userScore > 0) {// Filter Scored Anime
            ...
            let animeYear = anime?.seasonYear || anime?.startDate?.year || anime?.endDate?.year
            if (isaN(parseFloat(animeYear)) && includeYear) {
                year.push({ userScore: userScore, year: parseFloat(animeYear) })
            }
        }
    }
    ...
    // Add Extra Data
    if (filters['season']["current season"] === undefined) {
        filters['season']["current season"] = true
    }
    if (filters['season']["upcoming season"] === undefined) {
        filters['season']["upcoming season"] = true
    }
    ...
    if (!jsonIsEmpty(varScheme)) {
        ...
        for (let i = 0; i < animeEntries.length; i++) {
            ...
            let year = anime?.seasonYear || anime?.startDate?.year || anime?.endDate?.year
            ...
            if (typeof anime?.nextAiringEpisode?.episode === "number"
                ...
                status = "FINISHED"
            } else if (jsonIsEmpty(anime.nextAiringEpisode) && !(ncsCompare(status, "finished") || ncsCompare(status, "cancelled")) && year) {
                ...
                if (possibleAiringDate && possibleAiringDate > new Date()) {
                    anime.nextAiringEpisodeIsAdded = true
            ...
            recommendedAnimeList[animeID] = {
                ...
                animeRelations: anime?.relations?.edges,
                ...
                trailerThumbnailUrl: anime?.trailer?.thumbnail,
        ...
        recommendedAnimeListEntries = recommendedAnimeListEntries.map((anime) => {
            ...
            if (isJsonObject(anime?.nextAiringEpisode) && !anime?.nextAiringEpisodeIsAdded) {
