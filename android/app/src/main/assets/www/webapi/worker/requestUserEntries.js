            ...
            fetch('https://graphql.anilist.co', {
                ...
                .then((result) => {
                    ...
                    if (typeof (error = result?.errors?.[0]?.message) === "string") {
                        self.postMessage({ status: error || "Failed to check entries" })
                ...
                .catch((error) => {
                    ...
                    if (errorText === 'User not found' || errorText === 'Private User') {
                        ...
                        self.postMessage({ status: errorText })
                        self.postMessage({ error: errorText })
                    } else {
    ...
    function getUserEntries() {
        ...
        function recallAV(chunk) {
            fetch('https://graphql.anilist.co', {
                ...
                body: JSON.stringify({
                    query: `{MediaListCollection(userName:"${username}",chunk:${chunk},perChunk:${maxAnimePerChunk},forceSingleCompletedList:true,type:ANIME){hasNextChunk lists{entries{status media{id}score}}user{updatedAt}}}`
                })
            })
                .then(async (response) => {
                ...
                .then(({ result, headers }) => {
                    let error;
                    if (typeof (error = result?.errors?.[0]?.message) === "string") {
                        self.postMessage({ status: error || "Failed to retrieve entries." })
