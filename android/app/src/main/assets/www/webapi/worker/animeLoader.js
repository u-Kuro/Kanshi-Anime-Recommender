...
self.onmessage = async ({ data }) => {
    ...
    if (data?.checkStatus) {
        ...
        saveJSON(finalAnimeList, "finalAnimeList")
        saveJSON(data.hiddenEntries, "hiddenEntries");
        saveJSON(false, "shouldLoadAnime");
        ...
        finalAnimeList = recommendedAnimeList.filter((anime, idx) => {
            ...
            if (exclude.season["current season"]) {
                ...
                if (!isNaN(parseInt(year))) {
                    ...
                    if (typeof season === 'string') {
                        // is Current Season
                        if (season.toLowerCase() === currentSeasonYear?.season && parseInt(year) === currentSeasonYear?.year) {
                            return false
                        }
                    }
            ...
            if (!jsonIsEmpty(include.season)) {
                if (include.season["current season"] && include.season["upcoming season"]) {
                    ...
                    if (anime?.season?.toLowerCase?.() !== currentSeasonYear?.season || parseInt(year) !== currentSeasonYear?.year) {
                        // Get Season Dates
                        let season = anime?.season;
                        let { month, day } = anime?.startDate || {}
                        if (parseInt(month) > 0) { month = parseInt(month) - 1 }
        ...
        if (sortType === "desc") {
            if (sortName === "weighted score") {
                ...
                finalAnimeList.sort((a, b) => {
                    let x = Number(a?.favorites),
                        y = Number(b?.favorites);
                    if (!x) return 1;
                    if (!y) return -1;
                    return y - x
                })
...
function editAnimeToLoad(shownList = []) {
    return shownList.map((anime) => {
        ...
        // Get Shown Score
        let shownScore;
        let shownScoreName = customSortName || sortName
        if (shownScoreName === "score") {
            shownScore = formatNumber(score);
        } else if (shownScoreName === "user score") {
            shownScore = anime?.userScore;
        } else if (shownScoreName === "average score") {
