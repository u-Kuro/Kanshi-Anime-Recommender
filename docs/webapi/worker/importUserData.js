...
self.onmessage = async ({ data }) => {
    ...
    reader.onload = async () => {
        ...
        try {
            ...
            let shouldImportUserEntries = username !== undefined
            ...
            let shouldImportAniEntries = Object.keys((await retrieveJSON("animeEntries")) || {}).length < Object.keys(fileContent.animeEntries || {}).length
            ...
            let currentAnimeUpdate = await retrieveJSON("lastAnimeUpdate") || null
            if (shouldImportAniEntries) {
                let importingLastAnimeUpdate = new Date(1670770349 * 1000);
                if (lastAnimeUpdate instanceof Date && !isNaN(lastAnimeUpdate)) {
                    importingLastAnimeUpdate = lastAnimeUpdate
                } else if (currentAnimeUpdate instanceof Date && !isNaN(currentAnimeUpdate)) {
                    importingLastAnimeUpdate = currentAnimeUpdate
                }
                await saveJSON(importingLastAnimeUpdate, "lastAnimeUpdate")
            } else {
                currentAnimeUpdate = currentAnimeUpdate || new Date(1670770349 * 1000)
            ...
            let currentAiringAnimeUpdate = await retrieveJSON("lastAiringUpdateDate") || null
            if (shouldImportAniEntries) {
                let importingLastAiringUpdate = new Date(1670770349 * 1000);
                if (lastAiringUpdateDate instanceof Date && !isNaN(lastAiringUpdateDate)) {
                    importingLastAiringUpdate = lastAiringUpdateDate
                } else if (currentAiringAnimeUpdate instanceof Date && !isNaN(currentAiringAnimeUpdate)) {
                    importingLastAiringUpdate = currentAiringAnimeUpdate
                }
                await saveJSON(importingLastAiringUpdate, "lastAiringUpdateDate")
            } else {
                currentAiringAnimeUpdate = currentAiringAnimeUpdate || new Date(1670770349 * 1000)
            ...
            let tagCategoryInfo = fileContent.tagCategoryInfo ? new Date(fileContent.tagCategoryInfo) : null
            if (isJsonObject(tagCategoryInfo) && !jsonIsEmpty(tagCategoryInfo)) {
            ...
            let selectedCustomFilter = fileContent.selectedCustomFilter
            fileContent.selectedCustomFilter = null
            if (!activeTagFilters?.[selectedCustomFilter || ""]) {
                for (let customFilterNames in activeTagFilters) {
                    if (activeTagFilters?.[customFilterNames]?.["Anime Filter"]) {
                        selectedCustomFilter = customFilterNames
            ...
            let willSaveFilterOptions
            let filterOptions = fileContent.filterOptions || await retrieveJSON("filterOptions") || {}
            ...
            } else if (willSaveFilterOptions || isJsonObject(filterOptions)) {
                await saveJSON(filterOptions, "filterOptions")
            }

            self.postMessage({ progress: 94.08033826638479 })
            let animeEntries = fileContent.animeEntries
            let excludedEntries = fileContent.excludedEntries
            fileContent = null
            if (shouldImportAniEntries
                || (
                    lastAnimeUpdate instanceof Date
                    && !isNaN(lastAnimeUpdate)
                    && lastAnimeUpdate > currentAnimeUpdate
                    && isJsonObject(animeEntries)
                    && !jsonIsEmpty(animeEntries)
                )
            ) {
                await saveJSON(animeEntries, "animeEntries")
                await saveJSON(excludedEntries, "excludedEntries")
            }
            excludedEntries = animeEntries = null
    ...
    return await new Promise((resolve) => {
        ...
        function ParseError(_mes, _pos, _val, _cons) {
            ...
            if (!isNaN(_pos)) {
                ...
                let rpos = Math.max(0, _pos - 100)
                let strLen = Math.min(100, 100)
